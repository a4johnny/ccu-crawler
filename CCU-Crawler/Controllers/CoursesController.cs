...
namespace CCU_Crawler.Controllers
{
    public class CoursesController : Controller
    {
        ...
        [ValidateAntiForgeryToken] //@Html.AntiForgeryToken() 避免重複傳送
        public ActionResult Index(Course tempitem)
        {
                    ...
                    {   //xml path
                        ...
                        for (var i = 2; i < 30; i++)
                        {
                            ...
                            if (list2[0].SelectSingleNode(aaa) != null) // 回傳不是 null 就繼續抓
                            {
                                ...
                                if (db.Courses.Where(p => p.Name.Contains(a6)).FirstOrDefault() != null) // 跨領域課程目前被強制改版 先不使用
                                {
        ...
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(int id, string action)
        {
            if (action == "Delete") //單一刪除
            {
                Course crawler = db.Courses.Find(id);
                db.Courses.Remove(crawler);
                db.SaveChanges();
            }
            else if (action == "DeleteAll")//全部刪除
            {
                Course crawler = new Course();
                int count = db.Courses.ToList().Count; //求出共有幾筆
                while (count != 0) //砍到沒有為止
                {
                    Course crawler1 = db.Courses.Where(x => x.Id != null).FirstOrDefault(); // id 非null就砍
                    db.Courses.Remove(crawler1);
                    count--;
                    db.SaveChanges();
                }
            }
