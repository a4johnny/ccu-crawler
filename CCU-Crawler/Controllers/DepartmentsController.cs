...
namespace CCU_Crawler.Controllers
{
    public class DepartmentsController : Controller
    {
        ...
        public ActionResult Index(Department temp)
        {
            var web = new HtmlWeb
            {
        ...
        public ActionResult DeleteConfirmed(int id, string action)
        {
            ...
            {
                Department crawler = db.Departments.Find(id);
                db.Departments.Remove(crawler);
                db.SaveChanges();
            }
            else if (action == "DeleteAll")//全部刪除
            {
                Department crawler = new Department();
                int count = db.Departments.ToList().Count; //求出共有幾筆
                while (count != 0) //砍到沒有為止
                {
                    Department crawler1 = db.Departments.Where(x => x.Id != null).FirstOrDefault(); // id 非null就砍
                    db.Departments.Remove(crawler1);
                    count--;
                    db.SaveChanges();
                }
            }
